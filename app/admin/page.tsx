...
export default function AdminPage() {
  ...
  const [form, setForm] = useState({
    nombre: '', slug: '', avatar_color: '#f15922', avatar_emoji: '😊', rol: 'marketing',
  })
  ...
  async function guardar() {
    ...
    if (editando) {
      await supabase.from('mkt_usuarios').update({
        nombre: form.nombre, slug, avatar_color: form.avatar_color,
        avatar_emoji: form.avatar_emoji, rol: form.rol,
      }).eq('id', editando.id)
    } else {
      await supabase.from('mkt_usuarios').insert({
        nombre: form.nombre, slug, avatar_color: form.avatar_color,
        avatar_emoji: form.avatar_emoji, rol: form.rol, activo: true,
      })
    }
    await fetchUsuarios()
    setModalOpen(false)
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 700 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <button onClick={() => router.push('/')} style={{
              background: 'none', border: '1.5px solid #e8e8e8', borderRadius: '8px',
              padding: '0.375rem 0.75rem', cursor: 'pointer', fontSize: '0.85rem', color: '#555',
            }}>← Volver</button>
            <div>
              <h1 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#1a1a1a' }}>⚙️ Administración</h1>
              <p style={{ fontSize: '0.8rem', color: '#888', marginTop: '2px' }}>Gestión de usuarios del equipo</p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {usuarios.length === 0 && (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#888', backgroundColor: '#fff', border: '1.5px solid #e8e8e8', borderRadius: '12px' }}>
              <p style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>👥</p>
              <p style={{ fontWeight: 600 }}>No hay usuarios todavía</p>
              <p style={{ fontSize: '0.85rem', marginTop: '0.25rem' }}>Creá el primero para que pueda acceder al Hub.</p>
            </div>
          )}
          {usuarios.map(u => (
            <div key={u.id} style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {!u.activo && (
                    <span style={{ fontSize: '0.65rem', color: '#888', backgroundColor: '#f4f4f4', padding: '2px 8px', borderRadius: '99px' }}>inactivo</span>
                  )}
                </div>
          ...
          <div style={{
            ...
            <Campo label="Nombre">
              <input value={form.nombre}
                ...
                placeholder="Ej: Julieta García" style={inputStyle} />
            </Campo>
              ...
              <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                {EMOJIS.map(e => (
                  <button key={e} onClick={() => setForm(f => ({ ...f, avatar_emoji: e }))} style={{
                    ...
                    backgroundColor: form.avatar_emoji === e ? '#f9ddd3' : '#fff', cursor: 'pointer',
                  }}>{e}</button>
                ))}
              </div>
            </Campo>
              ...
              <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                {COLORES.map(c => (
                  <button key={c} onClick={() => setForm(f => ({ ...f, avatar_color: c }))} style={{
                    ...
                    border: form.avatar_color === c ? '3px solid #1a1a1a' : '2px solid transparent', cursor: 'pointer',
                  }} />
                ))}
              </div>
            </Campo>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              }}>{loading ? 'Guardando...' : editando ? 'Guardar cambios' : 'Crear usuario'}</button>
            </div>
          </div>
        </div>
      )}
